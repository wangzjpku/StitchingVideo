...
DWORD WINAPI StitchingAll(   )
{
	...
	Mat temp; 
#if 0	
	//LOGLN("display ROI with hight: " << (ResultStitch.rows * 0.15) <<" " << (ResultStitch.rows * 0.7)<< " " << ResultStitch.cols << " sec");  //  122  571
	if ((ResultStitch.rows * 0.15) < 10 )
		return 0;
	Mat disp = ResultStitch( Rect( 0  ,122  , ResultStitch.cols , 571 ));
	disp.convertTo( temp  , CV_8U  );

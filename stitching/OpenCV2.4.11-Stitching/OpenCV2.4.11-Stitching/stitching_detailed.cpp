...
DWORD WINAPI GetPreStruct( LPVOID lpParameter )
{
	...
	//{
		for (int i = 0; i < num_images ; ++i)
		{
			...
			//full_img_s[i] = sharedmat[i].clone();
...
DWORD WINAPI CaptureThread( LPVOID lpParameter ) 
{
 ...
 // resizeWindow("MyPicture" , 1000 , 300  );
  while(1) { 
	...
	for( int i = 0 ; i < num_images ; i++ )
	{
   		capture[i].read( sharedmat[i] );
	}
    imgready=true;

	Sleep(30);
//	Mat temp; 
//	sharedmat[1].convertTo( temp  , CV_8U  );
//	imshow("MyPicture", temp );
...
int OpenVideo()
{
	...
	filename.push_back("rtsp://169.254.49.14:554/user=admin&password=&channel=1&stream=0.sdp?real_stream");
	filename.push_back("rtsp://169.254.49.12:554/user=admin&password=&channel=1&stream=0.sdp?real_stream");
	filename.push_back("rtsp://169.254.49.11:554/user=admin&password=&channel=1&stream=0.sdp?real_stream");
	filename.push_back("rtsp://169.254.49.13:554/user=admin&password=&channel=1&stream=0.sdp?real_stream");
...
{  
    ...
    media = libvlc_media_new_location(instance, "rtsp://169.254.49.14:554/user=admin&password=&channel=1&stream=0.sdp?real_stream");  
...
//	HANDLE handleCaptureThread = CreateThread( NULL , 0 ,  CaptureThread  , NULL , 0 , NULL ); 
//	Sleep(20000);
...
//	CloseHandle(handleCaptureThread);

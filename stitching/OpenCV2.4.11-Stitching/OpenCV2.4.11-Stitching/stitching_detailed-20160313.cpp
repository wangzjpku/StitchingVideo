...
DWORD WINAPI GetPreStruct( LPVOID lpParameter )
{
	...
	{	
		...
		sprintf(testfile, ".\\output1\\img-%d.jpg", i );
		imwrite(testfile, full_img_s[i]);
...
//	LOGLN("Saving 31...");
	...
	//adjuster->~Estimator();
	cout << ba_refine_mask.size() << "  0  " << ba_refine_mask.capacity() << endl;
//	cout << features.size() << "  1  " << features.capacity() << endl;
//	cout << pairwise_matches.size() << "  2  " << pairwise_matches.capacity() << endl;
//	cout << cameras.size() << " 3   " << cameras.capacity() << endl;
...
//	cout << focals.size() << "  3  " << focals.capacity() << endl;
	...
	if (do_wave_correct)
	{
		...
		for (size_t i = 0; i < cameras.size(); ++i)
			cameras[i].R = rmats[i];
//		cout << rmats.size() << "  4  " << rmats.capacity() << endl;
	}
	...
	for (int img_idx = 0; img_idx < num_images; ++img_idx)
	{
		...
		//cout << mask_warped1.size() << " 5   " << mask_warped1.capacity() << endl;
		if( img_idx == 0 )
...
//	cout << result2.size() << "  6  " << result_mask2.size() << endl;
...
int UpdateMat()
{
	//WaitForSingleObject( hMutexRemap , INFINITE  );
	...
	//ReleaseMutex(hMutexRemap);

//	cout << xmapstitch.size() << " 7   " << xmapstitch.capacity() << endl;
//	cout << ymapstitch.size() << " 7   " << ymapstitch.capacity() << endl;
//	cout << img4.size() << " 7   " << img4.capacity() << endl;
...
DWORD WINAPI StitchingAll( LPVOID lpParameter )
{
	...
	int frmnum = 1;
	int meg;
	for(  frmnum = 2 ; frmnum < 120 ;   frmnum ++ )
	{
